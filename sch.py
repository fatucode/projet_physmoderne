...
# ====================================================================
# 3.2.5 : CONFRONTATION avec la theorie (cas V0=0), SANS tracer les paquets
#         (l'enonce : "la comparaison peut se faire sans representer")
# ====================================================================
def norme(col): return simpson(np.abs(col)**2, x)
 
print("--- 3.2.5  CONFRONTATION (V0 = 0) ---")
print(f"norme theorique attendue : 1.0000 (constante)")
for frac in (0.0, 0.25, 0.5, 1.0):
    idx = int(frac * n_stop)
    print(f"  t = {t[idx]:6.3f}  ->  norme numerique = {norme(psi[:,idx]):.3e}")
print("\nConclusion : la norme, qui devrait rester egale a 1, diverge.")
print("L'algorithme (derivee 1ere temps + 2nde espace) est instable pour Schrodinger.")
